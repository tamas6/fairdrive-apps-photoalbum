...
import { login, fileUpload, getDirectory, generateSeedPhrase, createAccount, getPods, openPod, downloadAllFiles } from "../store/services/fairOS";
...
export const applyMiddleware = (dispatch) => (action) => {
  switch (action.type) {
    ...
    case types.DOWNLOAD.DOWNLOAD_REQUEST:
      return downloadAllFiles(action.payload).then((res) => {
        dispatch({
          type: types.DOWNLOAD.DOWNLOAD_SUCCESS,
          payload: res,
        });
      })
